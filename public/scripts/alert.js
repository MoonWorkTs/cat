...
export const alert = (type = "err" | "ok", text) => {
  const alert = document.querySelector(".alert");

  if (alert.classList.contains("err") || alert.classList.contains("ok"))
    alert.classList.remove("err", "ok");

  alert.classList.add(type);
  alert.querySelector(".alert-text__p").innerHTML = text;

  if (type === "ok") alert.querySelector(".alert-icon").innerHTML = okSvg;
  if (type === "err") alert.querySelector(".alert-icon").innerHTML = errSvg;
